...
<!DOCTYPE html>
	...
	<header id="fh5co-header" class="fh5co-cover fh5co-cover-sm" role="banner" style="background-image:url(images/img_bg_1.jpg);height: 100px;margin-bottom:20px;">
		<div class="overlay">
		</div>
	</header>
			 ...
			 <?php
                ...
                $qry="SELECT * FROM request WHERE Reciever ='$email' ";
                //echo $qry;
                $rs=mysqli_query($conn,$qry);
                ...
                if(mysqli_num_rows($rs)>0)
                {
                    while($row=mysqli_fetch_assoc($rs))
                    {
			            $qry1="SELECT * FROM user WHERE Uid=".$row['Sender'];
				        $rs1=mysqli_query($conn,$qry1);
				        while($row1=mysqli_fetch_assoc($rs1))
		                {
            ?>                                         
					...
					<div class="col-md-12">
                        <div class="text-center">   
                        	...
                        	<div> 
	                        	...
	                        	<div>
	                        		<a href="acceptrequest.php?id=<?php echo $row1["Uid"]; ?>"><input type="button" class="btn btn-primary" name="btnaccept" value="Accept"></a>

	                        		<a href="rejectrequest.php?id=<?php echo $row1["Uid"]; ?>"><input type="button" class="btn btn-primary" name="btnrejact" value="Reject"></a>    
	                        	</div>
	                        </div>
                        </div>     
                    </div>
